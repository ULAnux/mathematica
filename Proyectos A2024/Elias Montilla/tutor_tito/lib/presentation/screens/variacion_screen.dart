...
class VariacionesScreen extends ConsumerStatefulWidget {
  const VariacionesScreen(
      {super.key,
      required this.data,
  ...
  @override
  VariacionesState createState() => VariacionesState();
}

class VariacionesState extends ConsumerState<VariacionesScreen> {
  ...
  void initState() {
    ...
    for (int i = 0; i < widget.variaciones.length; i++) {
      if (i % 3 == 0 && i != 0) {
        widget.variaciones[i].indice = index;
        ...
        if (widget.variaciones[i].fraseCompleta != null) {
          widget.variaciones[i].indice = index;
          expansionTileControllers.add(ExpansionTileController());
                  ...
                  content: SizedBox(
                    ...
                    child: ListView(
                      children: data.contenido.split('\n\n').map((e) {
                        ...
                        if (e.contains('{1FORM}')) {
                          return Center(
                              child: Image.asset('assets/repeticion_va.png'));
                        }
                        if (e.contains('{2FORM}')) {
                          return Center(
                              child: Image.asset('assets/no_repiten_va.png'));
                        }
                        return Padding(
...
class ExpanderActDos extends ConsumerWidget {
  ...
  final int index;
  final VariacionesScreen widget;
  final int i;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final desbloque = ref.watch(variacionesDesbloqueo);
    final actividadDos = ref.watch(resultadosActividaDosVaria);
    final completados = ref.watch(hoyResutosVaria);
      ...
      onExpansionChanged: (isExpanded) {
        ...
        if (isExpanded) {
          for (var controller in expansionTileControllers) {
            if (controller.isExpanded && controller != expansionTileControllers[widget.variaciones[i].indice]) {
              controller.collapse();
            }
          }
        }
            ...
            child: Container(
              ...
              Container( 
               ...
               : Column(children: [
                  ...
                  Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: widget.variaciones[0].nivel1A!.map((palabra) {
                    return Padding(
                  ...
                  if (nivelA1.isNotEmpty)
                    Container(
                      ...
                      SingleChildScrollView(
                        child: Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: widget.variaciones[0].nivel2A!.map((palabra) {
                  ...
                  if (nivelA2.isNotEmpty)
                    Container(
                        ...
                        child: nivelA2 == 'se repiten' ||
                            ...
                            ? Wrap(
                                children: widget.variaciones[0].nivel3A!
                                    .map((palabra) {
                                return GestureDetector(
                                  onTap: (){
                                    print('pala $palabra');
                            ...
                            : nivelA1 == 'Cuántas'
                                ? Row(
                                    mainAxisAlignment: MainAxisAlignment.center,
                                    children: widget.variaciones[0].nivel2B!
                                        .map((palabra) {
                                      return GestureDetector(
                                        onTap: (){
                        ...
                        : Row(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: widget.variaciones[0].nivel2C!
                                .map((palabra) {
                              return GestureDetector(
                                onTap: (){
                                  print(' esta $palabra');
...
class Expander extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final desbloque = ref.watch(variacionesDesbloqueo);
    final completados = ref.watch(hoyResutosVaria);
    final variacionesPuntaje = ref.watch(hoyResutosVariaPuntaje);
    int posicion = -1;
    ...
    return Padding(
      ...
      child: ExpansionTile(
        ...
        onExpansionChanged: (isExpanded) {
          ...
          if (isExpanded) {
            ...
            ref.read(actividaActual.notifier).state =
                widget.variaciones[i].palabras!;
            for (var controller in expansionTileControllers) {
              if (controller.isExpanded &&
                  controller !=expansionTileControllers[widget.variaciones[i].indice]) {
                controller.collapse();
              }
            }
          }
        ...
        enabled: desbloque > index,
        collapsedBackgroundColor: desbloque > index
            ? Color.fromARGB(255, 225, 139, 236)
            : const Color.fromARGB(255, 201, 201, 202),
        title: Row(
          children: [
             completados.contains(index) ? const Icon(Icons.check_box_outlined) : const Icon(Icons.check_box_outline_blank_outlined),
        ...
        children: [
                ...
                ? MaterialButton(
                    ...
                    onPressed: () {
                      ...
                      String cadena = encontrarMasSimilar(
                          ...
                          widget.variaciones[i].frasesCorrectras!);
                      // print('$cadena \n ${selecionarPalabras.join(' ')}');
                      ...
                      PuntajeDia ejercicioCompleto = PuntajeDia(ejercicio: index+1,fecha:  DateFormat('dd-MM-yyyy').format(DateTime.now()), tema: 'Variaciones',puntaje: '${calculo.ceil()}');
                      DBProvider.db.updatePermutacion(2, {'desbloquedo': desbloque});
                      DBProvider.db.nuevaPuntaje(ejercicioCompleto);

                      ref.read(hoyResutosVariaPuntaje.notifier).update((state) => [...state, '${calculo.ceil()}' ]);
                      ref.read(variacionesDesbloqueo.notifier).update((state) => state + 1);
                      ref.read(hoyResutosVaria.notifier).update((state) => [...state, index]);
          ...
          if (continua.isNotEmpty)
            Container(
              ...
              child: Center(
                  child: Text(
                'Estos son tus resultados: ${ widget.variaciones[i].indice < variacionesPuntaje.length ? variacionesPuntaje[widget.variaciones[i].indice] : '0' } de 100\nPuedes volver a evaluar mañana',
