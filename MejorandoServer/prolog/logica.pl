% **********************************************************************
% * logica.pl version alfa 1.0 2015-3-4                                *
% * Página web "logica.html" generada a partir de un código con HTML   *
% * termerizado en Prolog. Proyecto ULAnix mathematica.                *
% * Este programa es Software Libre. Desarrollado por Ixhel Mejías,    *
% * basándose en la página web diseñada e implementada por Santiago y  *
% * Sánchez (2014), disponible en:                                     *
...
busqueda --> html(form([style='align: left; margin: 0px 0px 0px 100px;',action='/pr', method='POST'], [
		...
		section(p(div([class='form-group col-xs-11 col-sm-11 col-md-11 col-lg-11 degraded container-planes'], p(['AQUÍ VAN TODAS LAS PREGUNTAS DE LÓGICA',p('Pregunta 1'),p('Pregunta 2'),p('Pregunta 3')]))))
...
foot --> html(footer([p(div([id='subir',class='text-right'],input([name='img-subir', type='image',  src='http://s15.postimg.org/82q42vo5j/icon_top.png', onclick='window.location.href = "#top";', alt='Subir']))),
		     ...
		     p(div([class='text-center'],'Diseñado por Carlos Santiago y Luis Sanchez. Generado desde Prolog por Ixhel Mejías.')),
