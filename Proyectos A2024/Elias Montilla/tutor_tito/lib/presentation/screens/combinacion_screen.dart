...
class CombinacionesScreen extends ConsumerStatefulWidget {
  const CombinacionesScreen(
      {super.key,
      required this.data,
      required this.combinaciones,
      required this.desbloqueadaVariaciones});
  final List<Actividad> combinaciones;
  final NotasCompletas data;
  final int desbloqueadaVariaciones;
  @override
  CombinacionesState createState() => CombinacionesState();
}

class CombinacionesState extends ConsumerState<CombinacionesScreen> {
  ...
  void initState() {
    super.initState();
    desbloq = widget.desbloqueadaVariaciones;
    for (int i = 0; i < widget.combinaciones.length; i++) {
      if (i % 3 == 0 && i != 0) {
        widget.combinaciones[i].indice = index;
        ...
        if (widget.combinaciones[i].fraseCompleta != null) {
          widget.combinaciones[i].indice = index;
          expansionTileControllers.add(ExpansionTileController());
          actividades.add(Expander(
              desbloqueadaPermutaciones: widget.desbloqueadaVariaciones,
              expansionTileControllers: expansionTileControllers,
              index: index,
              widget: widget,
              i: i));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: appBarVariacion(context, widget.data),
...
PreferredSizeWidget appBarVariacion(
                  ...
                  content: SizedBox(
                    ...
                    child: ListView(
                      children: data.contenido.split('\n\n').map((e) {
                        ...
                        if (e.contains('{1FORM}')) {
                          return Center(
                              child: Image.asset('assets/combinanorep.png'));
                        }
                        if (e.contains('{2FORM}')) {
                          return Center(
                              child: Image.asset('assets/conbinacionrepeticion.png'));
                        }
                        return Padding(
...
class ExpanderActDos extends ConsumerWidget {
  ...
  final int index;
  final CombinacionesScreen widget;
  final int i;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final desbloque = ref.watch(combinacionesDesbloqueo);
    final actividadDos = ref.watch(resultadosActividaDosCom);
    final completados = ref.watch(hoyResutosCombi);
      ...
      onExpansionChanged: (isExpanded) {
        ...
        if (isExpanded) {
          for (var controller in expansionTileControllers) {
            if (controller.isExpanded && controller != expansionTileControllers[widget.combinaciones[i].indice]) {
              controller.collapse();
            }
          }
        }
            ...
            child: Container(
              ...
              Container( 
                    ...
                    Row(
                      ...
                      MaterialButton(
                        ...
                        onPressed: (){
                          ...
                          ref.read(resultadosActividaDosCom.notifier).state='';  
                          ...
                          ref.read(nivel2.notifier).state='';
                           ...
                           switch(finalDos){
                            ...
                            ref.read(hoyResutosCombiPuntaje.notifier).update((state) => [...state, '50' ]);
                          ...
                          ref.read(combinacionesDesbloqueo.notifier).update((state) => state + 1);
                          ref.read(hoyResutosCombi.notifier).update((state) => [...state, index]);
               ...
               : Column(children: [
                  ...
                  Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: widget.combinaciones[0].nivel1A!.map((palabra) {
                    return Padding(
                  ...
                  if (nivelA1.isNotEmpty)
                    Container(
                      ...
                      SingleChildScrollView(
                        child: Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: widget.combinaciones[0].nivel2A!.map((palabra) {
                                    ...
                                    children: widget
                                      ...
                                      return GestureDetector(
                                        onTap: (){
                                          ...
                                          ref.read(resultadosActividaDosCom.notifier).state = palabra;
                                        },
                                        child: Padding(
                                          padding: const EdgeInsets.all(8.0),
                                          child: Container(
                  ...
                  if (nivelA2.isNotEmpty)
                    Container(
                        ...
                        child: nivelA2 == 'se repiten' ||
                            ...
                            ? Wrap(
                                children: widget.combinaciones[0].nivel3A!
                                    .map((palabra) {
                                return GestureDetector(
                                  onTap: (){
                                    print('pala $palabra');
                            ...
                            : nivelA1 == 'Cuántas'
                                ? Row(
                                    mainAxisAlignment: MainAxisAlignment.center,
                                    children: widget.combinaciones[0].nivel2B!
                                        .map((palabra) {
                                      return GestureDetector(
                                        onTap: (){
                        ...
                        : Row(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: widget.combinaciones[0].nivel2C!
                                .map((palabra) {
                              return GestureDetector(
                                onTap: (){
                                  print(' esta $palabra');
...
class Expander extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final desbloque = ref.watch(combinacionesDesbloqueo);
    final completados = ref.watch(hoyResutosCombi);
    final permutacionPuntaje = ref.watch(hoyResutosCombiPuntaje);
    int posicion = -1;
    ...
    return Padding(
      ...
      child: ExpansionTile(
        ...
        onExpansionChanged: (isExpanded) {
          ...
          if (isExpanded) {
            ...
            ref.read(actividaActual.notifier).state =
                widget.combinaciones[i].palabras!;
            for (var controller in expansionTileControllers) {
              if (controller.isExpanded &&
                  controller !=expansionTileControllers[widget.combinaciones[i].indice]) {
                controller.collapse();
              }
            }
          }
        ...
        enabled: desbloque > index,
        collapsedBackgroundColor: desbloque > index
            ? Color.fromARGB(255, 255, 206, 160)
            : const Color.fromARGB(255, 201, 201, 202),
        title: Row(
          children: [
             completados.contains(index) ? const Icon(Icons.check_box_outlined) : const Icon(Icons.check_box_outline_blank_outlined),
        ...
        children: [
                ...
                ? MaterialButton(
                    ...
                    onPressed: () {
                      ...
                      String cadena = encontrarMasSimilar(
                          ...
                          widget.combinaciones[i].frasesCorrectras!);
                      // print('$cadena \n ${selecionarPalabras.join(' ')}');
                      ...
                      PuntajeDia ejercicioCompleto = PuntajeDia(ejercicio: index+1,fecha:  DateFormat('dd-MM-yyyy').format(DateTime.now()), tema: 'Combinaciones',puntaje: '${calculo.ceil()}');
                      DBProvider.db.updatePermutacion(3, {'desbloquedo': desbloque});
                      DBProvider.db.nuevaPuntaje(ejercicioCompleto);

                      ref.read(hoyResutosCombiPuntaje.notifier).update((state) => [...state, '${calculo.ceil()}' ]);
                      ref.read(combinacionesDesbloqueo.notifier).update((state) => state + 1);
                      ref.read(hoyResutosCombi.notifier).update((state) => [...state, index]);
          ...
          if (continua.isNotEmpty)
            Container(
              ...
              child: Center(
                  child: Text(
                'Estos son tus resultados: ${ widget.combinaciones[i].indice < permutacionPuntaje.length ? permutacionPuntaje[widget.combinaciones[i].indice] : '0' } de 100\nPuedes volver a evaluar mañana',
